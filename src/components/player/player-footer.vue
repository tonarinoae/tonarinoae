<template>
  <div class="d-toolbar-footer flex items-end" :class="{
      '!pb-20px': fullscreen,
      '!px-25px': fullscreen
    }">
    <div class="w-full relative" @click.stop>
      <div class="h-5px flex items-center justify-center mb-3">
        <div class="w-full group relative cursor-pointer" @mousemove="onMouseMoveScope" @mouseout="onMouseOut"
          @mousedown.prevent="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd"
          @click="onTouchStart($event), onTouchMove($event), onTouchEnd()" ref="progressRef">
          <div class="absolute bottom-0 w-full pt-16px px-2px group-hover:pt-22px" />

          <div class="w-full bg-#fff/20 h-3px relative rounded-sm group-hover:h-5px progress-bar before:hidden" :class="{
              '!before:block': timeHover !== null
            }" :data-time="timeHover !== null ? parseTime(timeHover) : undefined" :style="{
              '--before-pos-x': `${(timeHover / duration) * 100}%`
            }">
            <!-- preload resource -->
            <div class="absolute top-0 left-0 h-full bg-#fff/40" :style="{
      ...
      <div class="mx--2 mb--1 flex items-center justify-between">
        <div class="flex items-center">
          ...
          <player-volume v-model:volume="volume" :addNotice :addKeybinding />
        ...
        <div class="flex items-center justify-end">
          ...
          <q-btn flat round dens class="d-action" :disable="!supportPip" @click="pip = !pip">
            <i-fluent-picture-in-picture-exit-24-regular v-if="pip" class="size-1.7em" />
            <i-fluent-picture-in-picture-enter-24-regular v-else class="size-1.7em" />

            <player-tooltip>{{ pip ? "Phóng to (i)" : "Hình trong hình (i)"}}</player-tooltip>
          </q-btn>

          <q-btn flat round dense class="d-action" @click="fullscreen = !fullscreen">
            <i-fluent-full-screen-minimize-24-filled v-if="fullscreen" class="size-1.7em" />
            <i-fluent-full-screen-maximize-24-filled v-else class="size-1.7em" />

            <player-tooltip>{{ fullscreen ? "Thu nhỏ (f)" : "Toàn màn hình (f)"}}</player-tooltip>
          </q-btn>
        </div>
      </div>
    </div>
